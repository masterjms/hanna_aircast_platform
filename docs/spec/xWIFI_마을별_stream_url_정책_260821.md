...
> ⚠ **먼저 읽을 것 — §8(2026-08-24 개정)**
>
> 아래 §1~7은 ESP32측과 합의한 2026-08-21 원본이라 그대로 보존한다. 다만
> **mount 경로는 §8에서 `/live/<job_id>`로 바뀌었다**(마을 여러 곳을 한 방송으로
> 묶는 다중 대상 지원). §1~7 본문의 `/live/<village_id>/<job_id>`,
> `/live/all/<job_id>` 예시는 옛 형태다.
>
> **단말 동작 규정(§3.2 "받은 문자열을 그대로 쓴다")은 변경 없다** — 경로가
> 바뀌어도 펌웨어는 영향받지 않는다. 그것이 이 설계의 목적이었다.
...
## 8. 개정 (서버측, 2026-08-24) — mount 경로를 `/live/<job_id>`로 단순화

**단말 동작 규정(§1~5)은 그대로다.** 바뀐 것은 서버가 만드는 경로 형태 하나이며, §3.2의 "단말은 `stream_url` 문자열을 그대로 쓴다" 규칙 덕분에 **펌웨어에는 영향이 없다**.

### 무엇이 바뀌었나

```
이전   /live/<village_id>/<job_id>   ·   /live/all/<job_id>
이후   /live/<job_id>                 (단일·다중·전체 공통)
```

### 왜

운영에서 **마을 여러 곳을 한 방송으로 묶는 요구**가 확정됐다(마을 2곳 동시 라이브). 이때 경로에 마을을 담을 수가 없다 — 대상이 마을 1·2인 방송의 경로에 무엇을 넣어야 하는가에 답이 없다. `job_id`는 전역 유일하므로 그것만으로 mount가 겹치지 않고, 단일/다중/전체가 전부 한 규칙이라 특례가 사라진다.

"어느 마을 방송인가"는 URL이 아니라 이력(`broadcast_events.target_ids`)이 답한다. 애초에 마을이 여럿인 방송은 URL로 표현될 수 없다.

세션마다 mount를 나누는 §6의 근거는 그대로 유효하다 — 오히려 다중 마을 지원으로 더 중요해졌다.

### §7 "남은 확인 필요 사항" 해소

**서버측 동적 mount 생성 로직은 구현·검증 완료**다. LIVE_START 발행 **전에** Icecast source(HTTP PUT)를 연결해 mount를 먼저 세우고, 방송 종료 시 닫는다(유령 mount 없음). PC 데모(`Icecast_시작.bat`)의 고정 mount + 상시 소스 구조는 쓰지 않는다.

실측으로 확인한 Icecast 동작(2026-08-24):

| 확인 항목 | 결과 |
|---|---|
| mount 1개 → 청취자 N명 오디오 복제 | Icecast가 처리. 청취자 3명이 바이트 단위로 동일한 스트림 수신 |
| 방송 중간에 합류한 청취자 | 코덱 헤더부터 정상 수신(디코딩 가능) |
| 소스 종료 시 청취자 연결 | 함께 정리됨(유령 청취 없음) |
| mount 배타성 | 마운트당 소스 1개, 두 번째는 403 |
| mount 4개 동시 송출 | 정상(설정 상향 후) |

즉 **서버는 mount 하나에 오디오를 한 번만 밀어넣으면 되고, 단말이 몇 대든 복제는 Icecast가 한다.** 서버측 복제 로직은 불필요하다.

### 인프라 필수 조치 (같은 실측에서 발견)

Icecast 기본 설정이 `sources=2`, `clients=100`이라 **동시 라이브 방송 3건째부터 실패**한다. 커스텀 `icecast.xml`(`sources=20`, `clients=400`)이 실제로 적용되도록 다음을 고쳤다:

- 운영 compose의 마운트 경로가 `/etc/icecast2/icecast.xml`로 잘못돼 있었다 — 이미지가 읽는 경로는 `/etc/icecast.xml`. **커스텀 설정이 한 번도 적용된 적이 없었다.**
- 이미지 entrypoint가 그 파일에 비밀번호를 직접 써넣으므로 직접 마운트가 불가(`:ro`면 기동 실패, `rw`면 저장소 파일에 평문 기록). 템플릿을 읽기전용으로 넣고 컨테이너가 복사해 쓰도록 변경.
- 커스텀 xml의 경로가 Debian 계열(`icecast2`)이라 이 이미지와 불일치 → 이미지 기준으로 정정.
- 개발 compose는 아예 마운트를 안 하고 있었다 → 운영과 같은 설정을 쓰도록 추가(개발에서 재현이 안 되고 운영에서만 터지는 차이를 없앤다).
