...
- 관리자는 네 계층이다: `super_admin` > `sido_admin` > `sigungu_admin` > `village_admin`. 범위는 조직 트리(`organizations`)를 따르며 마을의 주소는 트리의 입력값이 아니다. [관리자 계층 설계](../spec/xWIFI_관리자_계층_설계_260908.md).
- `super_admin`은 모든 마을과 미배정 단말을 관리한다.
- `sido_admin`은 소속 시·도 기관과 그 하위 시·군 기관의 마을을, `sigungu_admin`은 소속 시·군 기관의 마을을 조회·제어한다.
- `village_admin`은 `user_villages`에 지정된 마을만 조회·제어한다.
- 신규 단말 등록·credential·단말 삭제·OTA·CONFIG·기관 관리는 `super_admin`만 한다.
