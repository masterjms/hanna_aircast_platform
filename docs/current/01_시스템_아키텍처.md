...
| scheduler | CONFIG 기동 즉시·주기 재발행, 만료 계정 정리, 자동방송 실행기(매분) | FastAPI 프로세스 내부 |
...
├─ STATUS write buffer
├─ APScheduler: CONFIG reconcile · account expiry · schedule runner
...
- 관리자는 세 역할이다: `super_admin` > `org_admin` > `village_admin`. 범위는 깊이 제한 없는 조직 트리(`organizations`)의 부분 트리를 따르며 마을의 주소는 트리의 입력값이 아니다. 기관 관리자끼리의 위아래는 트리 위치가 정한다. [관리자 계층 설계](../spec/xWIFI_관리자_계층_설계_260908.md) v2.
- `super_admin`은 모든 마을과 미배정 단말을 관리한다.
- `org_admin`은 소속 기관과 그 아래 모든 기관의 마을을 조회·제어하고, 그 안에서 기관·마을·계정을 만든다(자기 마디는 못 옮기고 못 지움).
- `village_admin`은 `user_villages`에 지정된 마을만 조회·제어한다.
- 신규 단말 등록·credential·단말 삭제·OTA·CONFIG·뿌리 기관 생성은 `super_admin`만 한다.
- 권한은 React 메뉴 숨김과 무관하게 FastAPI scope에서 강제한다. 역할 → 마을 집합 산출은 `app/core/authz.py` 한 곳이다.
...
- 스케줄 실행은 수동 방송과 같은 `start_file_broadcast` 경로를 쓴다(대상 해석·겹침 검사·`job_id` 발번 동일). 중복 실행 방지는 `schedule_runs` 유니크, 재시도는 없다. `docs/spec/xWIFI_스케줄_설계_260909.md`.
