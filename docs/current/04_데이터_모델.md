...
| 도메인 | 테이블 |
|---|---|
| 조직·권한 | `organizations`, `users`, `villages`, `zones`, `user_villages` |
| 단말 | `devices` |
| 파일 | `files`, `download_tokens` |
| 자동 방송 | `schedules`, `schedule_runs` |
| 이력 | `broadcast_events`, `device_events` |
| 시스템·비용 | `current_config`, `daily_cost_summary` |

`daily_cost_summary`는 스키마는 있지만 앱 기능이 아직 구현되지 않았다. OTA package 전용 테이블은 아직 없다.
...
| `role` | varchar(20) | 아니오 | CHECK `super_admin`, `org_admin`, `village_admin` (0018) | 권한 역할 |
| `organization_id` | integer | 예 | FK `organizations.id` ON DELETE RESTRICT | `org_admin`의 소속 기관. 다른 역할은 NULL |
| `expires_at` | timestamptz | 예 | 인덱스 | 사용 기간의 끝. NULL이면 무기한 |
| `must_change_password` | boolean | 아니오 | `false` (0019) | 임시 비밀번호를 받고 아직 바꾸지 않음. true면 `/api/auth/me`·`/api/auth/password`·로그아웃 외 모든 API가 `PASSWORD_CHANGE_REQUIRED` |
| `created_at` | timestamptz | 아니오 | `now()` | 생성 시각 |

규칙:

- 범위는 역할마다 다르게 풀린다(`app/core/authz.resolve_scope`, [관리자 계층 설계](../spec/xWIFI_관리자_계층_설계_260908.md) §4): `super_admin` 전체, `org_admin` 소속 기관과 그 아래 모든 기관의 마을(부분 트리 — `app/core/orgtree.py`가 기관 전체를 질의 한 번으로 읽어 메모리에서 계산), `village_admin` `user_villages`. 기관 관리자끼리는 트리 위치가 위아래를 정하고, 같은 마디의 관리자는 서로 만지지 못한다.
...
- `expires_at`이 지난 계정은 로그인과 기존 토큰이 모두 401로 막히고, 매시간 도는 정리 작업이 계정을 지운다. 생성·수정 API는 일수(1~30, 기본 15)를 받아 요청 시각 기준으로 이 값을 다시 계산한다. 만료된 `super_admin`이 마지막 한 명이면 삭제하지 않고 로그인만 막는다.
- 계정을 삭제하면 `files.uploaded_by`, `broadcast_events.triggered_by`, `schedules.created_by`가 NULL이 된다(ON DELETE SET NULL). 이력은 불변 로그이므로 행 자체는 남는다. 계정별 파일함·이력·스케줄 분리는 아직 없다.

### 4.1a `organizations`

관리 기관 — 권한 트리의 마디다. 깊이 제한이 없고(v2, 0018) 마을은 어느 마디에나 붙는다. 권한은 이 트리를 따르고 마을의 주소는 트리의 입력값이 아니다. 마디는 이름과 부모뿐이다 — 행정구역인지 자체 묶음인지 구분하지 않는다.

| 컬럼 | 타입 | NULL | 기본/제약 | 의미 |
|---|---|---:|---|---|
| `id` | integer | 아니오 | PK | 기관 ID |
| `name` | varchar(100) | 아니오 |  | 이름 (예: 경기도, 안양시, 만안구, 북부권역) |
| `parent_id` | integer | 예 | FK `organizations.id` ON DELETE RESTRICT, 인덱스 | 상위 마디. NULL이면 뿌리. 자기/후손을 부모로 두는 순환은 API가 `ORG_CYCLE`로 막는다 |
| `created_at` | timestamptz | 아니오 | `now()` | 생성 시각 |

하위 기관·소속 마을·계정이 하나라도 있으면 API가 `ORGANIZATION_IN_USE`로 삭제를 막고 FK가 한 번 더 막는다. 자동방송(`schedules.target_ids`)이 가리키는 기관·마을·단말은 `SCHEDULE_TARGET_IN_USE`로 삭제를 막는다 — `target_ids`는 JSONB라 FK가 없어서 API가 본다. 트리 모양을 바꾸는 쓰기(기관 추가·옮기기·삭제, 마을·기관 관리자를 기관에 붙이기)는 트랜잭션 어드바이저리 락(`orgtree.lock_tree`)으로 한 줄로 선다 — 동시 옮기기로 순환이 생기지 않게. 0015의 `level`·`jurisdiction_code`는 0018에서 삭제됐다.
...
| `organization_id` | integer | 예 | FK `organizations.id` ON DELETE RESTRICT, 인덱스 | 이 마을을 관리하는 기관. `b_code`가 "어디 있나"라면 이건 "누가 하나". NULL이면 `super_admin`만 봄 |
...
| `uploaded_by` | integer | 예 | FK `users.id` ON DELETE SET NULL | 등록 사용자 |
| `created_at` | timestamptz | 아니오 | `now()` | 생성 시각 |
...
- 참조 중인 schedule이 있으면 API가 `FILE_IN_USE`로 거부하며 어느 스케줄인지 함께 알려준다. **방송 이력은 삭제를 막지 않는다**(0017) — `broadcast_events.file_id`가 NULL이 되고 「무엇을」은 이력이 들고 있는 `file_name`이 계속 보여준다.
...
자동방송 규칙 한 줄. 실행 날짜는 저장하지 않고 `app/modules/schedule/rules.py`가 계산한다. [스케줄 설계](../spec/xWIFI_스케줄_설계_260909.md).

| 컬럼 | 타입 | NULL | 기본/제약 | 의미 |
|---|---|---:|---|---|
| `id` | integer | 아니오 | PK | 스케줄 ID |
| `repeat` | varchar(10) | 아니오 | CHECK `daily`, `weekly`, `monthly`, `yearly` | 반복 종류 |
| `weekdays` | integer[] | 예 | weekly 만 | 0=일~6=토 |
| `month_days` | integer[] | 예 | monthly 만 | 1~31. 그 날이 없는 달은 건너뜀 |
| `year_dates` | jsonb | 예 | yearly 만 | `[{"month","day"}]`. 2월 29일은 윤년에만 |
| `once_date` | date | 예 | once 만 (0020) | 그 날짜(KST)의 `fire_time`에 한 번. 지나면 다시 걸리지 않고 다음 실행은 null. 지난 시각은 API가 거절 |
| `fire_time` | time | 아니오 |  | 하루 중 시각. KST |
| `file_id` | integer | 아니오 | FK `files.id` | 방송 파일 |
| `target_scope` | varchar(20) | 아니오 | CHECK `village`, `device`, `organization` | 대상 범위 |
| `target_ids` | jsonb | 아니오 | 기본 `[]` | 마을 id / MAC 목록 / 기관 id |
| `store_flash` | boolean | 아니오 | 기본 false | 단말 플래시 저장 |
| `enabled` | boolean | 아니오 | 기본 true | 사용 여부 |
| `created_by` | integer | 예 | FK `users.id` ON DELETE SET NULL | 생성 사용자 |
| `created_at` | timestamptz | 아니오 | `now()` | 생성 시각 |

`organization` 대상은 실행 시점에 기관(자기 + 바로 아래 기관)의 마을로 펼친다. 그래서 `broadcast_events`에는 `village`로 남고 이력·겹침 검사는 그대로다.

### 7.2 `schedule_runs`

규칙이 실제로 걸린 회차의 기록. 미래는 저장하지 않는다.

| 컬럼 | 타입 | NULL | 기본/제약 | 의미 |
|---|---|---:|---|---|
| `id` | bigint | 아니오 | PK | |
| `schedule_id` | integer | 아니오 | FK `schedules.id` ON DELETE CASCADE, 인덱스 | |
| `fire_at` | timestamptz | 아니오 | UNIQUE(`schedule_id`, `fire_at`) | 규칙상 나갔어야 하는 시각 |
| `status` | varchar(20) | 아니오 |  | `pending` → `started` \| `skipped` \| `failed` |
| `reason` | varchar(200) | 예 |  | 건너뜀·실패 사유 |
| `event_id` | bigint | 예 | FK `broadcast_events.id` ON DELETE SET NULL | started 면 만들어진 방송 |
| `created_at` | timestamptz | 아니오 | `now()` | |

유니크 제약이 중복 실행 방지의 실체다. 실행기는 방송을 걸기 전에 이 행을 넣고, 충돌이면 물러난다. 서버 재시작·같은 분의 이중 tick·다중 워커 모두 안전하다. INSERT 직후 프로세스가 죽으면 `pending`으로 남고 나가지 않는다 — 두 번 나가는 것보다 한 번 빠지는 것이 낫다.
...
| `file_id` | integer | 예 | FK `files.id` ON DELETE SET NULL | 방송한 파일. 파일이 지워지면 NULL |
| `file_name` | varchar(255) | 예 |  | 방송 시작 시점의 파일명 스냅샷. 파일을 지워도 이력에 남는다 |
| `schedule_id` | integer | 예 | FK `schedules.id` | 자동 방송 원인 |
| `triggered_by` | integer | 예 | FK `users.id` ON DELETE SET NULL | 수동 실행 사용자 |
...
| `live_bitrate_kbps` | smallint | 아니오 | 기본 24 | 라이브 opus 비트레이트 |
| `file_bitrate_kbps` | smallint | 아니오 | 기본 24 | 파일함 mp3 비트레이트 |
...
| `file_wait_sec` | 10~60 |
| `live_bitrate_kbps` | 16 또는 24 |
| `file_bitrate_kbps` | 16 또는 24 |

공통 CONFIG의 세 필드가 바뀔 때만 `config_version`을 올린다. 서버 내부 방송 대기시간과 오디오 비트레이트는 단말 공통 설정이 아니므로 버전을 올리지 않는다.
...
현재 head는 `0020`이다(`schedules.once_date`, repeat `once`).
