...
| `role` | varchar(20) | 아니오 | CHECK `super_admin`, `sido_admin`, `sigungu_admin`, `village_admin` | 권한 역할 |
| `organization_id` | integer | 예 | FK `organizations.id` ON DELETE RESTRICT | `sido_admin`·`sigungu_admin`의 소속 기관. 다른 역할은 NULL |
| `expires_at` | timestamptz | 예 | 인덱스 | 사용 기간의 끝. NULL이면 무기한 |
| `created_at` | timestamptz | 아니오 | `now()` | 생성 시각 |

규칙:

- 범위는 역할마다 다르게 풀린다(`app/core/authz.resolve_scope`, [관리자 계층 설계](../spec/xWIFI_관리자_계층_설계_260908.md) §4): `super_admin` 전체, `sido_admin` 소속 기관과 하위 기관의 마을, `sigungu_admin` 소속 기관의 마을, `village_admin` `user_villages`.
...
관리 기관 — 시·도청과 시·군청의 두 단계 트리다. 권한은 이 트리를 따르고 마을의 주소는 트리의 입력값이 아니다.

| 컬럼 | 타입 | NULL | 기본/제약 | 의미 |
|---|---|---:|---|---|
| `id` | integer | 아니오 | PK | 기관 ID |
| `name` | varchar(100) | 아니오 |  | 기관 이름 (예: 금산군청) |
| `level` | varchar(20) | 아니오 | CHECK `sido`, `sigungu` | 수준 |
| `parent_id` | integer | 예 | FK `organizations.id` ON DELETE RESTRICT, 인덱스 | `sigungu`의 상위 `sido`. `sido`는 항상 NULL |
| `jurisdiction_code` | varchar(5) | 예 |  | 법정동코드 앞자리(시도 2 / 시군구 5). 마을 생성 때 기관 제안용, 권한 판정에는 안 씀 |
| `created_at` | timestamptz | 아니오 | `now()` | 생성 시각 |

소속 마을·계정·하위 기관이 하나라도 있으면 API가 `ORGANIZATION_IN_USE`로 삭제를 막고 FK가 한 번 더 막는다.
