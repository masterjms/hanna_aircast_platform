...
async def list_schedules(db: Db, scope: Scope) -> list[ScheduleOut]:
    """내 범위와 겹치는 스케줄. editable 로 고칠 수 있는지 알려준다."""
    return await service.list_schedules(db, scope)
...
async def occurrences(
    ...
    return await service.occurrences(db, scope, start, end)


@router.get("/{schedule_id}", response_model=ScheduleOut)
async def get_schedule(schedule_id: int, db: Db, scope: Scope) -> ScheduleOut:
    return await service.get_schedule(db, schedule_id, scope)
...
async def delete_schedule(schedule_id: int, db: Db, scope: Scope) -> None:
    await service.delete_schedule(db, schedule_id, scope=scope)


@router.get("/{schedule_id}/runs", response_model=list[ScheduleRunOut])
async def list_runs(schedule_id: int, db: Db, scope: Scope) -> list[ScheduleRunOut]:
    """최근 실행 결과 — 나갔다·건너뛰었다·실패했다와 그 사유."""
    return await service.list_runs(db, schedule_id, scope)
